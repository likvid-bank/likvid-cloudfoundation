...
locals {
  landing_zones = {
    ...
    prod = {
      ...
      quotas = [
        { key = "limits.cpu", value = 2000 },
        { key = "requests.cpu", value = 1000 },
        { key = "limits.memory", value = 2048 },
        { key = "requests.memory", value = 1024 },
        { key = "requests.storage", value = 5 },
        { key = "persistentvolumeclaims", value = 8 },
      ]
    }
  }
}
...
resource "meshstack_landingzone" "this" {
  ...
  spec = {
    ...
    info_link                     = "https://likvid-bank.github.io/likvid-cloudfoundation/platforms/aks/landingzones/${each.key}.html"
