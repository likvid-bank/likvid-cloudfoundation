...
terraform {
  source = "https://github.com/meshcloud/collie-hub.git//kit/azure/buildingblocks/postgresql/backplane?ref=main"
...
generate "config" {
  path      = "${get_terragrunt_dir()}/../postgresql.test/config.tf"
  if_exists = "overwrite"
  contents  = <<EOF
terraform {
  backend "azurerm" {
