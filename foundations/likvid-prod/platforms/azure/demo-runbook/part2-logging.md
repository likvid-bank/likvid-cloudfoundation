...
> - [Activity Logs in LAW](https://portal.azure.com/#@703c8d27-13e0-4836-8b2e-8390c588cf80/resource/subscriptions/e4a6af88-cd23-4785-acd6-d7221f755be7/resourceGroups/law-rg-likvid-foundation/providers/Microsoft.OperationalInsights/workspaces/log-analytics-workspace/logs)
...
## Proposed demo change — Add Policy log category

**Current state:** Only `Administrative` Activity Log category is forwarded to the LAW.

**Why:** Management group diagnostic settings only support `Administrative` and `Policy` — all
other categories (Security, ServiceHealth, Alert, etc.) are subscription-scope only and are
rejected by the API at MG scope. Adding `Policy` captures every Azure Policy evaluation event
(audit, deny, deployIfNotExists remediation) from every subscription — directly relevant for a
governance workshop.

### Step 1 (kit): make log categories configurable

Edit [`kit/azure/logging/main.tf`](../../../../../kit/azure/logging/main.tf) — extend the
`azapi_resource` diagnostic setting to accept a variable list of categories:
...
### Step 2 (foundation): enable Administrative + Policy
