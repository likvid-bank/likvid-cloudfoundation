...
# order an ephemeral building block against it. This is a TENANT_LEVEL block, so `fixtures` still
# carries the target tenant (mesh_tenant_id) the service account is created in; the e2e module
# rejects setting both `bbd_version_ref` and `fixtures.*` in a way that selects build-from-source.
# See the meshstack-hub `e2e-test` skill for the invocation protocol.
