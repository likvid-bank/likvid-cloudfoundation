...
locals {
  ...
  md_contents = {
    ...
    replace(basename(f), ".md", "") => templatefile("${path.module}/guides/${f}", {
      ...
      meshstack_project_sap_core_platform = "[${meshstack_project.sap_core_platform.spec.display_name}](${var.meshpanel_base_url}/#/w/${meshstack_project.sap_core_platform.metadata.owned_by_workspace}/p/${meshstack_project.sap_core_platform.metadata.name})",
