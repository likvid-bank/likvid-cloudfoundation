...
resource "google_project_iam_custom_role" "cloudfoundation_deploy_billing" {
  role_id     = "${replace(var.deploy_principal.name, "-", "_")}_meshplatform_billing"
...
resource "google_project_iam_member" "cloudfoundation_deploy_billing" {
  project = var.cloud_billing_export_project_id
  role    = google_project_iam_custom_role.cloudfoundation_deploy_billing.id
  member  = var.deploy_principal.id
}
