...
#   program = ["bash", "-c", <<EOT
#     curl --location --request POST 'https://api.ionos.com/reseller/v2/contracts' \
#     --header 'Authorization: ${var.token}' \
#     --header 'Content-Type: application/json' \
#     --data-raw '{
#         "name": "co-${var.workspace_identifier}-${var.project_identifier}",
#         "resellerReference": "${var.reseller_reference}",
...
#             "ips": ${var.ips}
#         }
#     }' | jq -c '{id: .id, href: .href, name: .name, resellerReference: .resellerReference}'
#   EOT
#   ]
# }
...
#   program = ["bash", "-c", <<EOT
#     curl --location --request POST 'https://api.ionos.com/reseller/v2/contracts/${data.external.create_contract.result.id}/admins' \
#     --header 'Authorization: Basic ${var.token}' \
#     --header 'Content-Type: application/json' \
#     --data-raw '{
#         "firstName": "${var.first_name}",
...
#         "email": "${var.email}",
#         "password": "${var.password}"
#     }' | jq -c '{id: .id, href: .href, firstName: .firstName, lastName: .lastName, email: .email}'
#   EOT
#   ]
# }
