...
run "verify" {
  variables {
    ...
    parent_management_group = "likvid-corp" # note: the subscription needs to live in likvid-corp because we use it also for connectivity scenario tests
  }
  ...
  assert {
    condition = (
      data.azurerm_management_group.lz.display_name == "likvid-corp"
