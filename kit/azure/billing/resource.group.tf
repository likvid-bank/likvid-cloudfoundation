...
resource "azuread_group" "billing_admins" {
  ...
  description = "Privileged Cloud Foundation group. Members can manage billing profiles, reserved instances and have full access to all cost data."
  security_enabled = true
}

resource "azuread_group" "billing_readers" {
  display_name     = var.billing_reader_group
  description = "Privileged Cloud Foundation group. Members can read all cost data."
  security_enabled = true
}
