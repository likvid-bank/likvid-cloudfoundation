...
|[${module.policy_corp.policy_assignments["Deny-HybridNetworking"].display_name}]|Deny|${module.policy_corp.policy_assignments["Deny-HybridNetworking"].description}|By denying these resources, we can ensure that all traffic to and from the resource is routed through the private network, which is more secure.|
