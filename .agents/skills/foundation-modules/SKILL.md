...
# Plus the `generate "smoke_tfvars"` block that writes `test_context` to smoke.auto.tfvars.json.
# The `hub` skill owns that snippet and its fields — see § "This foundation's side of the protocol:
# foundation mode".
```
